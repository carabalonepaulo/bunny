...
template RAYLIB_VERSION*(): auto = "4.2-dev"
...
type Mesh* {.bycopy.} = object
    ...
    boneIds*: uint8 # Vertex bone ids, max 255 bone ids, up to 4 bones influence by vertex (skinning)
...
type rAudioProcessor* {.bycopy.} = object
...
# File path list
type FilePathList* {.bycopy.} = object
...
proc SetWindowState*(flags: uint32) {.RLAPI, importc: "SetWindowState".} # Set window configuration state using flags (only PLATFORM_DESKTOP)
...
proc SetWindowOpacity*(opacity: float32) {.RLAPI, importc: "SetWindowOpacity".} # Set window opacity [0.0f..1.0f] (only PLATFORM_DESKTOP)
...
proc GetRenderWidth*(): int32 {.RLAPI, importc: "GetRenderWidth".} # Get current render width (it considers HiDPI)
proc GetRenderHeight*(): int32 {.RLAPI, importc: "GetRenderHeight".} # Get current render height (it considers HiDPI)
...
proc GetMonitorWidth*(monitor: int32): int32 {.RLAPI, importc: "GetMonitorWidth".} # Get specified monitor width (current video mode used by monitor)
proc GetMonitorHeight*(monitor: int32): int32 {.RLAPI, importc: "GetMonitorHeight".} # Get specified monitor height (current video mode used by monitor)
...
proc EnableEventWaiting*() {.RLAPI, importc: "EnableEventWaiting".} # Enable waiting for events on EndDrawing(), no automatic event polling
proc DisableEventWaiting*() {.RLAPI, importc: "DisableEventWaiting".} # Disable waiting for events on EndDrawing(), automatic events polling
...
proc WaitTime*(seconds: float64) {.RLAPI, importc: "WaitTime".} # Wait for some time (halt program execution)
...
proc SetRandomSeed*(seed: uint32) {.RLAPI, importc: "SetRandomSeed".} # Set the seed for the random number generator
...
proc ExportDataAsCode*(data: cstring; size: uint32; fileName: cstring): bool {.RLAPI, importc: "ExportDataAsCode".} # Export data to code (.h), returns true on success
...
proc GetFileLength*(fileName: cstring): int32 {.RLAPI, importc: "GetFileLength".} # Get file length in bytes (NOTE: GetFileSize() conflicts with windows.h)
...
proc GetApplicationDirectory*(): cstring {.RLAPI, importc: "GetApplicationDirectory".} # Get the directory if the running application (uses static string)
proc ChangeDirectory*(dir: cstring): bool {.RLAPI, importc: "ChangeDirectory".} # Change working directory, return true on success
proc IsPathFile*(path: cstring): bool {.RLAPI, importc: "IsPathFile".} # Check if a given path is a file or a directory
proc LoadDirectoryFiles*(dirPath: cstring): FilePathList {.RLAPI, importc: "LoadDirectoryFiles".} # Load directory filepaths
proc LoadDirectoryFilesEx*(basePath: cstring; filter: cstring; scanSubdirs: bool): FilePathList {.RLAPI, importc: "LoadDirectoryFilesEx".} # Load directory filepaths with extension filtering and recursive directory scan
proc UnloadDirectoryFiles*(files: FilePathList) {.RLAPI, importc: "UnloadDirectoryFiles".} # Unload filepaths
proc IsFileDropped*(): bool {.RLAPI, importc: "IsFileDropped".} # Check if a file has been dropped into window
proc LoadDroppedFiles*(): FilePathList {.RLAPI, importc: "LoadDroppedFiles".} # Load dropped filepaths
proc UnloadDroppedFiles*(files: FilePathList) {.RLAPI, importc: "UnloadDroppedFiles".} # Unload dropped filepaths
proc GetFileModTime*(fileName: cstring): int32 {.RLAPI, importc: "GetFileModTime".} # Get file modification time (last write time)
# Compression/Encoding functionality
proc CompressData*(data: UncheckedArray[byte]; dataSize: int32; compDataSize: pointer): uint8 {.RLAPI, importc: "CompressData".} # Compress data (DEFLATE algorithm), memory must be MemFree()
proc DecompressData*(compData: UncheckedArray[byte]; compDataSize: int32; dataSize: pointer): uint8 {.RLAPI, importc: "DecompressData".} # Decompress data (DEFLATE algorithm), memory must be MemFree()
proc EncodeDataBase64*(data: UncheckedArray[byte]; dataSize: int32; outputSize: pointer): ptr char {.RLAPI, importc: "EncodeDataBase64".} # Encode data to Base64 string, memory must be MemFree()
proc DecodeDataBase64*(data: UncheckedArray[byte]; outputSize: pointer): uint8 {.RLAPI, importc: "DecodeDataBase64".} # Decode Base64 string data, memory must be MemFree()
...
proc GetMouseWheelMove*(): float32 {.RLAPI, importc: "GetMouseWheelMove".} # Get mouse wheel movement for X or Y, whichever is larger
proc GetMouseWheelMoveV*(): Vector2 {.RLAPI, importc: "GetMouseWheelMoveV".} # Get mouse wheel movement for both X and Y
...
proc GetTouchPointId*(index: int32): int32 {.RLAPI, importc: "GetTouchPointId".} # Get touch point identifier for given index
proc GetTouchPointCount*(): int32 {.RLAPI, importc: "GetTouchPointCount".} # Get number of touch points
# ------------------------------------------------------------------------------------
...
proc DrawLineBezierCubic*(startPos: Vector2; endPos: Vector2; startControlPos: Vector2; endControlPos: Vector2; thick: float32; color: Color) {.RLAPI, importc: "DrawLineBezierCubic".} # Draw line using cubic bezier curves with 2 control points
...
proc CheckCollisionPointLine*(point: Vector2; p1: Vector2; p2: Vector2; threshold: int32): bool {.RLAPI, importc: "CheckCollisionPointLine".} # Check if point belongs to line created between two points [p1] and [p2] with defined margin in pixels [threshold]
...
proc GetImageColor*(image: Image; x: int32; y: int32): Color {.RLAPI, importc: "GetImageColor".} # Get image pixel color at (x, y) position
...
proc LoadFontEx*(fileName: cstring; fontSize: int32; fontChars: pointer; glyphCount: int32): Font {.RLAPI, importc: "LoadFontEx".} # Load font from file with extended parameters, use NULL for fontChars and 0 for glyphCount to load the default character set
...
proc ExportFontAsCode*(font: Font; fileName: cstring): bool {.RLAPI, importc: "ExportFontAsCode".} # Export font as code file, returns true on success
...
proc DrawTextCodepoints*(font: Font; codepoints: ptr ptr int; count: int32; position: Vector2; fontSize: float32; spacing: float32; tint: Color) {.RLAPI, importc: "DrawTextCodepoints".} # Draw multiple character (codepoint)
...
proc TextCodepointsToUTF8*(codepoints: ptr ptr int; length: int32): ptr char {.RLAPI, importc: "TextCodepointsToUTF8".} # Encode text as codepoints array into UTF-8 text string (WARNING: memory must be freed!)
...
proc DrawCubeTextureRec*(texture: Texture2D; source: Rectangle; position: Vector3; width: float32; height: float32; length: float32; color: Color) {.RLAPI, importc: "DrawCubeTextureRec".} # Draw cube with a region of a texture
...
proc DrawCylinderEx*(startPos: Vector3; endPos: Vector3; startRadius: float32; endRadius: float32; sides: int32; color: Color) {.RLAPI, importc: "DrawCylinderEx".} # Draw a cylinder with base at startPos and top at endPos
...
proc DrawCylinderWiresEx*(startPos: Vector3; endPos: Vector3; startRadius: float32; endRadius: float32; sides: int32; color: Color) {.RLAPI, importc: "DrawCylinderWiresEx".} # Draw a cylinder wires with base at startPos and top at endPos
...
proc DrawBillboardPro*(camera: Camera; texture: Texture2D; source: Rectangle; position: Vector3; up: Vector3; size: Vector2; origin: Vector2; rotation: float32; tint: Color) {.RLAPI, importc: "DrawBillboardPro".} # Draw a billboard texture defined by source and rotation
...
proc DrawMeshInstanced*(mesh: Mesh; material: Material; transforms: ptr ptr Matrix; instances: int32) {.RLAPI, importc: "DrawMeshInstanced".} # Draw multiple mesh instances with material and different transforms
...
proc LoadModelAnimations*(fileName: cstring; animCount: uint32): ptr ModelAnimation {.RLAPI, importc: "LoadModelAnimations".} # Load model animations from file
...
proc UnloadModelAnimations*(animations: ptr ModelAnimation; count: uint32) {.RLAPI, importc: "UnloadModelAnimations".} # Unload animation array data
...
proc SetSoundPan*(sound: Sound; pan: float32) {.RLAPI, importc: "SetSoundPan".} # Set pan for a sound (0.5 is center)
...
proc LoadWaveSamples*(wave: Wave): float32 {.RLAPI, importc: "LoadWaveSamples".} # Load samples data from wave as a 32bit float data array
...
proc LoadMusicStreamFromMemory*(fileType: cstring; data: UncheckedArray[byte]; dataSize: int32): Music {.RLAPI, importc: "LoadMusicStreamFromMemory".} # Load music stream from data
...
proc SeekMusicStream*(music: Music; position: float32) {.RLAPI, importc: "SeekMusicStream".} # Seek music to a position (in seconds)
...
proc SetMusicPan*(music: Music; pan: float32) {.RLAPI, importc: "SetMusicPan".} # Set pan for a music (0.5 is center)
...
proc SetAudioStreamPan*(stream: AudioStream; pan: float32) {.RLAPI, importc: "SetAudioStreamPan".} # Set pan for audio stream (0.5 is centered)
...
proc SetAudioStreamCallback*(stream: AudioStream; callback: AudioCallback) {.RLAPI, importc: "SetAudioStreamCallback".} # Audio thread callback to request new data
proc AttachAudioStreamProcessor*(stream: AudioStream; processor: AudioCallback) {.RLAPI, importc: "AttachAudioStreamProcessor".} 
proc DetachAudioStreamProcessor*(stream: AudioStream; processor: AudioCallback) {.RLAPI, importc: "DetachAudioStreamProcessor".} 
const KEY_MENU* = 82